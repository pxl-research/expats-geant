...
- [ ] 1.1 Add `trafilatura` to `requirements.txt` (pin a known-good minor version)
- [ ] 1.2 Add `CUE_WEB_INGEST_ENABLED=false` to `.env.example` with a comment
  describing privacy implications
- [ ] 1.3 Update `docs/DEPLOYMENT.md` env-var table with the new flag

## 2. Cue API — fetch + extraction core

- [ ] 2.1 Create `cue_api/web_fetch.py` with a `fetch_url(url)` function:
  10 s timeout, polite User-Agent, max 5 redirects, size cap from
  `max_file_size_mb`; returns `(final_url, content_type, body_bytes,
  initial_url)`
- [ ] 2.2 Add a content-type router that dispatches HTML to Trafilatura and
  binary/text types to the existing MarkItDown path
- [ ] 2.3 Trafilatura extraction wrapper: `favor_precision=True`,
...
- [ ] 2.4 Compute `likely_js_rendered` heuristic: extracted text length < 200
  chars on an HTML response of > 5 KB
- [ ] 2.5 In-process preview cache, 60 s TTL, keyed by
  `(session_id, normalised_url)`; normalisation strips fragment, lowercases
  scheme/host, keeps query string
- [ ] 2.6 Unit tests: HTML happy path (Wikipedia or fixture HTML),
...
- [ ] 3.1 Create `cue_api/routes/web.py` with `POST /web/preview` and
  `POST /web/ingest`
- [ ] 3.2 Both endpoints check `app.state.web_ingest_enabled` and return HTTP
  403 with a clear message when disabled
- [ ] 3.3 Both endpoints check per-session opt-in flag and return HTTP 403 when
  the session has not granted consent
- [ ] 3.4 `/web/preview` returns: `final_url`, `initial_url`, `title`,
...
- [ ] 4.1 In `cue_api/ingest.py`, add a helper that ingests pre-extracted text
  under a given source name + `source_url` metadata (reuses existing chunking
  + embedding)
- [ ] 4.2 Add a `delete_by_source_url(store, source_url)` helper that removes
  all chunks tagged with the given URL; used by re-ingest overwrite
- [ ] 4.3 Source name derivation: HTML → page title (truncated to 100 chars)
...
- [ ] 6.1 Update `docs/CUE_API.md` with the two endpoints, request/response
  shapes, error codes, and the operator flag
- [ ] 6.2 Update `docs/OPERATOR_RUNBOOK.md` with a "Should I enable web
