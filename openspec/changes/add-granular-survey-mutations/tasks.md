...
- [ ] 1.1 Create `shape_api/mutations.py` with typed error classes:
...
- [ ] 1.2 Implement `apply_init_survey(survey: Survey) -> Survey`. Idempotent
  pass-through (validation handled upstream by Pydantic).
- [ ] 1.3 Implement `apply_add_section(survey, section, after_id=None) -> Survey`.
...
- [ ] 1.10 Unit tests in `tests/test_shape_mutations.py`: one happy-path
  and one error-path test per mutation function. ~14 tests.
- [ ] 1.11 Unit test: `add_question` preserves the supplied id (move
  case).
- [ ] 1.12 Unit test: `delete_section` removes the section's questions
  along with it.
- [ ] 1.13 Unit test: `update_section` rejects a body containing
  `questions` with `InvalidPatch`.

## 2. Section-size warnings in validation

- [ ] 2.1 Locate the function that produces validation issues for a
...
- [ ] 2.2 Add a stronger-worded warning with code `section_overlong`
  when any section has > 50 questions (mutually exclusive with
  `section_dense`).
- [ ] 2.3 Unit tests: 30-question section → no new warning;
  31-question → `section_dense`; 51-question → `section_overlong`,
  no `section_dense`.

## 3. LLM tool surface

- [ ] 3.1 In `shape_api/tools.py`, define OpenAI-style tool schemas for
...
- [ ] 3.2 Extend `dispatch_tool_call(name, arguments, base_path, session_id)`
...
- [ ] 4.1 In `shape_api/models.py`, add request body models:
  ...
  mutation endpoint.
- [ ] 4.2 In `shape_api/routes/chat.py`, add six new endpoints
  ...
  `MutationResponse`.
- [ ] 4.3 Wire the rate limiter (`@limiter.limit("…/minute")`) on the
  mutation endpoints matching the existing PUT endpoint's policy.
- [ ] 4.4 Integration tests in `tests/test_chat_conversational_api.py`
...
- [ ] 5.1 In `shape_api/conversation.py`, remove `_SURVEY_TAG_RE` and
  `_parse_chat_response`.
- [ ] 5.2 Remove the "Output the COMPLETE survey every time" instruction
  and the full inline JSON schema block from `build_system_prompt`.
- [ ] 5.3 Add a concise tool overview to the system prompt: one
...
- [ ] 5.4 Remove the "I also noticed: …" post-processing block from
...
- [ ] 5.5 Change `MAX_TOOL_CALL_ITERATIONS` from 3 to 25. Update the
  comment above the constant.
- [ ] 5.6 Update `execute_chat_turn` so `survey_updated` becomes
...
- [ ] 6.2 Add a test for a multi-tool turn: model issues
  `add_section` + `add_question` + `add_question` in parallel; the
  draft contains the new section with both questions after the turn;
  `survey_updated: true`.
- [ ] 6.3 Add a test for the error-recovery loop: first tool response
  is `section_not_found`; second LLM iteration corrects the id and
  succeeds; `survey_updated: true`.
- [ ] 6.4 Add a test for the move case: model issues `delete_question`
  + `add_question` with the same id and a new `section_id`; the
  question's id is preserved.
- [ ] 6.5 Add a test that a pure Q&A turn (no tool calls) returns
  `survey_updated: false` and does not modify the draft.
- [ ] 6.6 Keep the existing truncation-guard test
  `test_chat_turn_truncated_output_returns_clean_message` as-is — it
  still covers `init_survey` payload truncation.

## 7. Docs

- [ ] 7.1 If a docs file like `docs/SHAPE_API.md` exists, document the
  new endpoints with request/response shapes. Otherwise, skip — the
  Pydantic models are the contract.
- [ ] 7.2 Document the tool surface in `shape_api/tools.py`'s module
  docstring (names + one-line purpose each), so the prompt and the
  code agree.

## 8. Validation gates

- [ ] 8.1 Run `ruff check shape_api/ m_shared/ tests/` — clean.
- [ ] 8.2 Run `ruff format --check shape_api/ m_shared/ tests/` — clean.
- [ ] 8.3 Run `.venv/bin/python -m pytest tests/ --no-cov -q` — green.
- [ ] 8.4 Run `openspec validate add-granular-survey-mutations --strict`
  — clean.
