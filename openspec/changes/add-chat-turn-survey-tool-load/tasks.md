...
- [ ] 1.1 In `m_shared/llm/client.py`, add a new method
  ...
  `temperature` logic.
- [ ] 1.2 Leave `create_completion(...)` (the content-only method) untouched.
- [ ] 1.3 Unit test: a mocked completion that returns content only → method
  returns a result with `content` populated and no tool calls.
- [ ] 1.4 Unit test: a mocked completion that returns a tool call → method
  returns a result with the tool call surfaced (function name, arguments,
  call ID).

## 2. Shape — tool definition and dispatcher

- [ ] 2.1 Create `shape_api/tools.py` with:
...
- [ ] 2.2 Unit test: dispatcher returns the on-disk draft JSON for an existing
  session.
- [ ] 2.3 Unit test: dispatcher returns a sentinel JSON (`{}` or
  `{"survey": null}` — pick and document) when the session has no draft yet.
- [ ] 2.4 Unit test: dispatcher raises `ValueError` for an unknown tool name.

## 3. Shape — summary enrichment

- [ ] 3.1 In `shape_api/suggestion_engine.py`, modify `compact_survey_summary`
  to format sections as `Section [<sec_id>]: <title>` and questions as
  `- [<q_id>] <text>`. Preserve indentation. No other fields.
- [ ] 3.2 Unit test: a survey with two sections, three questions each → the
  ...
  metadata keys.
- [ ] 3.3 Audit: confirm `compact_survey_summary` is only called from the
...
- [ ] 4.1 In `shape_api/conversation.py`, refactor `execute_chat_turn` to use
  a loop with at most 3 iterations (named constant `MAX_TOOL_CALL_ITERATIONS`).
- [ ] 4.2 On each iteration: call the LLM via the new full-message method,
  passing the `[get_full_survey]` tool definition.
- [ ] 4.3 If the response message has tool calls: append the assistant tool-call
...
- [ ] 4.4 If the response message has no tool calls: exit the loop; parse the
...
- [ ] 4.5 If the loop reaches the cap: treat the last assistant message as the
...
- [ ] 4.6 Log an `INFO`-level structured entry for each successful tool call
  including session ID and an iteration counter.
- [ ] 4.7 When a `<survey_update>` block is applied and the loop never invoked
  `get_full_survey` on this turn, log a `WARNING` so we can monitor
  non-compliance.

## 5. Shape — prompt update

- [ ] 5.1 In `shape_api/conversation.py:build_system_prompt`, add a paragraph
...
- [ ] 5.2 Keep the existing soft instruction about "only include
  `<survey_update>` when proposing structural changes."
- [ ] 5.3 Do **not** add language asserting the rule is enforced server-side
  (it is not).

## 6. Tests — integration

- [ ] 6.1 Mock LLM scripted to return a tool call on iteration 1 and a
  `<survey_update>` on iteration 2 → assert draft is saved with the LLM's
  posted survey content and `survey_updated=True`.
- [ ] 6.2 Mock LLM scripted to return text only on iteration 1 (no tool call,
  no `<survey_update>`) → assert single LLM call, no dispatch, no draft
  change, `survey_updated=False`.
- [ ] 6.3 Mock LLM scripted to return a `<survey_update>` on iteration 1
  without calling the tool → assert draft is saved (soft enforcement) **and**
  a `WARNING` is logged indicating non-compliance.
- [ ] 6.4 Mock LLM scripted to loop tool calls indefinitely → assert the loop
  stops at the cap, a `WARNING` is logged, and the turn returns gracefully.
- [ ] 6.5 Reproduce the clobber scenario (`/tmp/test_chat_clobber.py` shape):
...
- [ ] 7.1 Update `docs/SHAPE_API.md`: under `POST /chat/{session_id}`, add a
...
- [ ] 7.2 If a tunable `MAX_TOOL_CALL_ITERATIONS` is exposed via env var, add
  it to `.env.example` and `docs/DEPLOYMENT.md`. Otherwise skip.

## 8. Validation

- [ ] 8.1 `openspec validate add-chat-turn-survey-tool-load --strict` clean.
- [ ] 8.2 Full test suite green (`pytest`).
